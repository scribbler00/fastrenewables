...
__all__ = ['flatten_ts', 'LinBnAct', 'GANMLP', 'GANCNN', 'GAN', 'WGAN']
...
class GANMLP(torch.nn.Module):
    def __init__(self, ann_structure, bn_cont=False, act_fct=torch.nn.ReLU, final_act_fct=nn.Sigmoid, embedding_module=None):
...
class GANCNN(torch.nn.Module):
    def __init__(self, ann_structure, bn_cont=False, act_fct=nn.ReLU, final_act_fct=nn.Sigmoid, embedding_module=None):
        super(GANCNN, self).__init__()

        self.model = TemporalCNN(cnn_structure=ann_structure, batch_norm_cont=bn_cont, cnn_type='cnn', act_func=act_fct, final_activation=final_act_fct)
...
#todo
class WGAN(GAN):
    def __init__(self, generator, discriminator, gen_optim, dis_optim, n_z=100, clip=0.001):
        super(WGAN, self).__init__(generator, discriminator, gen_optim, dis_optim, n_z, clip)
        self.generator = generator
        self.discriminator = discriminator
        self.gen_optim = gen_optim
        self.dis_optim = dis_optim
