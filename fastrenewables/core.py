...
# Cell
def read_files(
    files: PosixPath, key: str = "/powerdata", key_metadata=None, sep: str = ";"
) -> pd.DataFrame:
...
class NormalizePerTask(TabularProc):
    ...
    def setups(self, to: Tabular):
        print("i have been here 1")
        self.means = (
            getattr(to, "train", to)[to.cont_names + "TaskID"].groupby("TaskID").mean()
        )
        self.stds = (
            getattr(to, "train", to)[to.cont_names + "TaskID"]
            .groupby("TaskID")
            .std(ddof=0)
            + 1e-7
        )
    ...
    def encode(self, to):
        ...
        for task_id in to.items[self.task_id_col].unique():
            to.conts.loc[to.loc[:, self.task_id_col] == task_id] = (
                (
                    to.conts[to.loc[:, self.task_id_col] == task_id]
                    - self.means.loc[task_id]
                )
                / self.stds.loc[task_id]
            ).describe()
...
class FilterByCol(TabularProc):
    ...
    def encodes(self, to):
        ...
        if not self.keep:
            mask = ~mask
        to.items.drop(to.items[mask].index, inplace=True)
        if self.drop_col_after_filter:
            to.items.drop(self.col_name, axis=1, inplace=True, errors="ignore")
...
class TabularRenewables(CollBase, GetAttr, FilteredBase):
    def __init__(
        ...
        self.cont_names, self.cat_names = cont_cat_split(
            self.dfs[0], dep_var=y_names, max_card=1000
        )
        if not include_task_id:
            self.cat_names = [c for c in self.cat_names if c != self.task_id_col]
        ...
        if splits is not None:
            splits = splits(range_of(merged_df))
    ...
    def subset(self, i):
        return self.to.new(
            self.items[
                slice(0, self.to.split)
                if i == 0
                else slice(self.to.split, len(self.to))
            ]
        )

    def copy(self):
        self.items = self.to.copy()
        return self

    def decode(self):
        return self.to.procs.decode(self.to)

    def decode_row(self, row):
        return self.to.new(pd.DataFrame(row).T).decode().items.iloc[0]

    def show(self, max_n=10, **kwargs):
        self.to.show(max_n, **kwargs)

    def setup(self):
        self.to.procs.setup(self.to)
    ...
    def targ(self):
        return self.to.items[self.y_names]

    def x_names(self):
        return self.to.cat_names + self.to.cont_names

    def n_subsets(self):
        return 2

    def y(self):
        return self.to[self.to.y_names[0]]

    def new_empty(self):
        return self.new(pd.DataFrame({}, columns=self.to.items.columns))
    ...
    def all_col_names(self):
        ...
        return (
            self.to.x_names + self.to.y_names
            if len(ys) == len(self.to.y_names)
            else self.to.x_names
        )
